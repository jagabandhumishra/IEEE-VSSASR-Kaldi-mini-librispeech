...
if [ $stage -le 1 ]; then
  local/data_lm.sh $data data/local/lm
fi
...
if [ $stage -le 6 ]; then
cat data/train_clean_5/text| cut -d ' ' -f 2- >data/train_clean_5/trans
while read line
do
echo "<s> $line </s>" >> data/train_clean_5/lm_train.txt
done <data/train_clean_5/trans
fi
